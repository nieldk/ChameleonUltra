...
Complete reference for the Phreakbyte edition ChameleonUltra client (`chameleon_cli_main.py`). Auto-generated from the CLI source: 26 command groups, 136 commands.
...
- `-q`, `--qty` — Number of pages to read before giving up (default: auto-detect the tag's size, or read until an empty response / Terminator TLV is found if the size can't be determined).
...
- `-q`, `--qty` — Number of available user pages on the tag, used as a safety check before writing (default: auto-detect the tag's size).
...
Import a Flipper Zero `.nfc` dump file into a MIFARE Ultralight / NTAG emulator slot

- `-s`, `--slot` <1-8> — Slot Index: [1, 2, 3, 4, 5, 6, 7, 8] Default: active slot (choices: 1, 2, 3, 4, 5, 6, 7, 8)
- `-f`, `--file` — Flipper Zero `.nfc` file to import (required)
- `--amiibo` — Automatically derive the PWD/PACK pair from the tag UID (Amiibo-style NTAG215 dumps)
...
Parse a saved DESFire dump/trace file and print its contents

- `-f`, `--file` — File to parse (required)
- `--hexdump` — Also print a raw hex dump of the file contents

#### `hf des eload`

Load DESFire emulator data into a slot

- `-s`, `--slot` <1-8> — Slot Index: [1, 2, 3, 4, 5, 6, 7, 8] Default: active slot (choices: 1, 2, 3, 4, 5, 6, 7, 8)
- `-f`, `--file` — File to load emulator data from (required)

#### `hf des edump`

Dump DESFire emulator data from a slot

- `-s`, `--slot` <1-8> — Slot Index: [1, 2, 3, 4, 5, 6, 7, 8] Default: active slot (choices: 1, 2, 3, 4, 5, 6, 7, 8)
- `-f`, `--file` — File to write emulator data to (default: print to screen)

#### `hf des einfo`

Show DESFire emulator configuration for a slot

- `-s`, `--slot` <1-8> — Slot Index: [1, 2, 3, 4, 5, 6, 7, 8] Default: active slot (choices: 1, 2, 3, 4, 5, 6, 7, 8)

#### `hf des eblank`

Reset a slot's DESFire emulator data to a blank card

- `-s`, `--slot` <1-8> — Slot Index: [1, 2, 3, 4, 5, 6, 7, 8] Default: active slot (choices: 1, 2, 3, 4, 5, 6, 7, 8)
- `-u`, `--uid` — Set a specific UID on the blanked card (default: random/factory UID)

#### `hf des estats`

Show DESFire emulator statistics (e.g. authentication attempt counters)
...
- `--pm3` — write each session as a Proxmark3 .trace (-NN.trace)
