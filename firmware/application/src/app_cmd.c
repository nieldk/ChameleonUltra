...
/* Reboot into bootloader in UF2 mode.
 *
 * The new UF2 bootloader uses the same dfu_enter_check() in
 * nrf_bootloader.c, which requires:
 *     (GPREGRET & 0xF9) == 0xB1
 * Bit 1 (0x02) is reserved for UF2-mode signalling and is masked out
 * of that check, so it survives into the bootloader where our patched
 * main() / nrf_dfu_uf2.c can read it. Final GPREGRET = 0xB3.
 *
 * Both transports (CDC + MSC) come up either way; the UF2 bit is only
 * needed if we ever want to suppress CDC when entered via this path. */
static data_frame_tx_t *cmd_processor_enter_bootloader_uf2(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
#define BOOTLOADER_DFU_UF2_BIT_MASK             (0x02)
#define BOOTLOADER_DFU_UF2_START                (BOOTLOADER_DFU_START | BOOTLOADER_DFU_UF2_BIT_MASK)
    APP_ERROR_CHECK(sd_power_gpregret_clr(0, 0xffffffff));
    APP_ERROR_CHECK(sd_power_gpregret_set(0, BOOTLOADER_DFU_UF2_START));
    nrf_pwr_mgmt_shutdown(NRF_PWR_MGMT_SHUTDOWN_GOTO_DFU);
    while (1) __NOP();
    return data_frame_make(cmd, STATUS_SUCCESS, 0, NULL);
}
 ...
 *
 * The application carries an embedded copy of a freshly-built bootloader.bin
 * (see firmware/application/src/embedded_bootloader.h, generated by
 * tools/gen_embedded_bl.py from objects/bootloader.hex).
 *
 * On invocation we:
 *   1. Run a pre-flight check (CRC32 + size) of the embedded data.
 *      Failure here returns an error code WITHOUT touching flash.
 *   2. Disable the SoftDevice. Raw NVMC writes to BL pages would
 *      otherwise HardFault.
 *   3. Erase the BL region, write the embedded bootloader, reset.
 *
 * Once SoftDevice is down, USB CDC is also gone, so the host won't
 * receive a reply. The CLI sees a disconnect — that's the success
 * signal. Reconnect after the new bootloader has booted.
 *
 * Risks: power loss between erase and full write = brick. Recoverable
 * only via SWD. Don't unplug the device for a few seconds after
 * issuing this command. */
static data_frame_tx_t *cmd_processor_update_bl(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    /* Pre-flight: validate the embedded blob before destroying anything. */
    bl_updater_status_t st = bl_updater_validate();
    if (st != BL_UPDATER_OK) {
        return data_frame_make(cmd, STATUS_PAR_ERR, sizeof(st), (uint8_t *)&st);
    }

    /* bl_updater_run() disables SoftDevice and resets. Doesn't return. */
    bl_updater_run();
...
static cmd_data_map_t m_data_cmd_map[] = {
    ...
    {    DATA_CMD_ENTER_BOOTLOADER_UF2,         NULL,                        cmd_processor_enter_bootloader_uf2,          NULL                   },
