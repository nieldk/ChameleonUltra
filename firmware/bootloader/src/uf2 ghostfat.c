 ...
 *
 * Provides INFO_UF2.TXT (always) and FAIL.TXT (after rejected blocks)
 * in the root directory for transfer diagnostics.
 *
 * BUILD-TIME FLAG: UF2_ALLOW_BL_WRITE
 *   When defined, the bounds check is widened to allow writes into the
 *   bootloader region (0xF3000 - 0xFE000) in addition to the normal app
 *   region. MBR params (0xFE000+) and BL settings page (0xFF000+) remain
 *   protected. This is intended ONLY for personal test builds — for
 *   public releases keep this undefined so end users can't accidentally
 *   brick their devices by dragging the wrong UF2.
 *
 *   When UF2_ALLOW_BL_WRITE is set, writes into the BL region are
 *   verified by reading back after the NVMC operation. This catches
 *   silent failures from BPROT or other hardware-level write blocks
 *   that would otherwise appear as "successful" transfers that didn't
 *   actually change anything.
 *
 *   You're flashing the BL while it's executing. The nRF52840 has no
 *   instruction cache that would protect against this — if a UF2 block
 *   erases the page containing currently-executing code, behavior is
 *   undefined. In practice this usually still works because the
 *   prefetcher carries the few instructions needed to complete the NVMC
 *   write, but treat each BL write as one-shot: don't expect the BL to
 *   keep functioning afterwards. Plan to reset before doing anything
 *   else.
 *
 * MIT License.
 */
...
/* Status files in the first two data clusters. */
...
/* Writable flash range. Public-safe by default; widen via build flag. */
#ifdef UF2_ALLOW_BL_WRITE
    /* Personal test build: allow writes up to the start of MBR params.
     * BL region (0xF3000 - 0xFE000) is now writable.
     * MBR params (0xFE000) and BL settings (0xFF000) remain protected. */
    #define UF2_FLASH_WRITE_END   0x000FE000UL
    /* Anything at or above this address is in the BL region, which
     * requires verify-after-write to catch BPROT silent failures. */
    #define UF2_FLASH_BL_START    UF2_FLASH_APP_END
#else
    /* Public-safe build: BL region locked, normal apps only. */
    #define UF2_FLASH_WRITE_END   UF2_FLASH_APP_END
#endif
...
int uf2_ghostfat_write_block(uint32_t lba, const uint8_t *buf)
{
    ...
    if (b->target_addr < UF2_FLASH_APP_START ||
        b->target_addr + b->payload_size > UF2_FLASH_WRITE_END) {
        uf2_status_record_rejected(b->block_no, b->num_blocks,
...
#ifdef UF2_ALLOW_BL_WRITE
    /* For writes into the BL region, verify the bytes actually landed
     * in flash. This catches silent failures from BPROT or other
     * hardware-level write blocks that would otherwise look like
     * successful transfers. App-region writes don't need this — they
     * either work or the device wouldn't have booted into this BL. */
    if (b->target_addr >= UF2_FLASH_BL_START) {
        if (memcmp((const void *)(uintptr_t)b->target_addr,
                   b->data, b->payload_size) != 0) {
            uf2_status_record_rejected(b->block_no, b->num_blocks,
